...
public class Helper {
	/* Helper methods
	 * Honestly, most of these methods should be in GameMap.java
	 */
	// Make constructor with nearestShipsByDistance to save processing time
	...
	public static Ship getClosestDockedEnemyWithNoAllies(GameMap gameMap, Entity entity) {
		...
		for (Entry<Double, Ship> entry : nearbys.entrySet()) {
			...
			if (ship.getOwner() != gameMap.getMyPlayerId() && ship.getDockingStatus() != Ship.DockingStatus.Undocked) {
				if (Helper.filterOutDockedShips(gameMap, Helper.getEnemiesNear(gameMap, ship, 10)).size() > 0) {
					continue;
				}
				return ship;
	...
	public static Ship getClosestDockedEnemyWithLeastAllies(GameMap gameMap, Entity entity) {
		...
		for (Entry<Double, Ship> entry : nearbys.entrySet()) {
			...
			if (ship.getOwner() != gameMap.getMyPlayerId() && ship.getDockingStatus() != Ship.DockingStatus.Undocked) {
				int tempAround = Helper.filterOutDockedShips(gameMap, Helper.getEnemiesNear(gameMap, ship, 10)).size();
