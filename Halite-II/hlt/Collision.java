...
public class Collision {
    ...
    public static boolean segmentCircleIntersect(final Position start, final Position end, final Entity circle, final double fudge) {
    	...
    	// Fixed collision by storing next moves of ships that have went and instead of using two parametric equations, use one by fixing the frame of reference to the other ship
    	if (circle instanceof Ship && ((Ship) circle).getNewPos() != null) {
    		alreadyX = ((Ship) circle).getNewPos().getXPos() - circle.getXPos();
    		alreadyY = ((Ship) circle).getNewPos().getYPos() - circle.getYPos();
    	}
